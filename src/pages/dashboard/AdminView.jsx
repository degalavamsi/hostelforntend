...
const AdminView = ({ 
    ...
    return (
        <div className="space-y-8 w-full animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-10">
            ...
            <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[48px] p-10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)]">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 bg-primary-600 rounded-[20px] shadow-2xl shadow-primary-900/40">
                            <Settings2 className="w-7 h-7 text-white" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-black text-white tracking-tighter">Infrastructure Hub</h2>
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Core Utility Matrix</p>
                        </div>
                    </div>
                    <a href="/utilities" className="group">
                        <div className="flex items-center gap-3 px-6 py-3 bg-white/5 border border-white/10 rounded-2xl text-primary-400 font-bold text-xs uppercase tracking-widest hover:bg-primary-600 hover:text-white transition-all hover:border-primary-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {[
                        ...
                        <div key={i} className={`bg-white/5 border border-white/10 rounded-3xl p-6 hover:border-${item.color}-500/30 transition-all group relative overflow-hidden`}>
                             ...
                             <div className="flex items-center gap-3 mb-6 relative z-10">
                                ...
                                <h3 className="text-white font-black text-xs uppercase tracking-tight">{item.label}</h3>
                             </div>
                             <div className="flex items-end justify-between relative z-10">
                                <h4 className="text-4xl font-black text-white tracking-tighter">{item.val}</h4>
                                <p className="text-[9px] text-slate-500 font-black uppercase tracking-tight mb-1">{item.subtitle}</p>
                             </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Resolution Desk */}
                <div className="lg:col-span-2 glass-card rounded-[40px] p-8 flex flex-col h-[500px] relative overflow-hidden group">
                    ...
                    <div className="space-y-4 overflow-y-auto pr-4 custom-scrollbar flex-1 relative z-10">
                        {complaints.map(c => (
                            <div key={c._id} className="p-6 bg-white/5 border border-white/10 rounded-3xl hover:bg-white/10 hover:border-primary-500/30 transition-all group/item shadow-2xl flex flex-col gap-4">
                                <div className="flex justify-between items-start">
                                    <div className="space-y-1">
                                        <h4 className="text-white font-black text-sm tracking-tight group-hover/item:text-primary-400 transition-colors uppercase">{c.title}</h4>
                                        <div className="flex items-center gap-3">
                                            <p className="text-[9px] font-black text-slate-500 tracking-widest uppercase">Student ID: {c.student_name || 'Anonymous'}</p>
                                        </div>
                                    </div>
                                    <span className={`px-2.5 py-1 rounded-full text-[8px] font-black uppercase tracking-widest shadow-xl border ${
                        ...
                        {complaints.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center text-slate-800 space-y-6">
                                ...
                                <p className="text-3xl font-black italic tracking-widest opacity-20 text-center uppercase">Zero Pending Pulse</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {/* Broadcast Feed */}
                <div className="glass-card rounded-[40px] p-8 flex flex-col h-[500px] relative overflow-hidden group">
                     <div className="absolute -right-10 -bottom-10 w-64 h-64 bg-amber-500/5 blur-[100px]" />
                     <h2 className="text-xl font-black text-white mb-8 flex items-center gap-3 uppercase tracking-tighter shrink-0 relative z-10">
                     ...
                     <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1 relative z-10">
                        {notices.map(n => (
                            <div key={n._id} className={`p-6 border rounded-3xl flex flex-col transition-all hover:scale-[1.02] ${n.priority === 'urgent' ? 'bg-rose-500/5 border-rose-500/20 shadow-lg shadow-rose-900/10' : 'bg-primary-500/5 border-primary-500/20'}`}>
                                ...
                                <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center text-[8px] font-black uppercase tracking-widest">
                        ...
                        {notices.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center text-slate-800 space-y-6 opacity-20">
                                ...
                                <p className="text-2xl font-black italic tracking-widest uppercase">Broadcast Silent</p>
                            </div>
                        )}
                     </div>
                </div>
